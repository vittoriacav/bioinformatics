...
"""I insert in the sequence1 two spaces that will accomodate the first two additional columns in the matrix (first with 
sequence2 and second with zeros)"""
seq1.insert(0, " ")
seq1.insert(0, " ")
...
rowS before sequence2 starts (first with sequence1 and second with zeros)"""
n = len(seq1)
m = len(seq2)+2
...
in the second the zeros and all the other with a 0 that will be filled later with scores"""
i = 2
while i < m:
    row = [seq2[i-2]]
    for j in range(n-1):
...
following the Smith & Waterman scoring method and using score values decided at the beginning"""
start = 0
for i in range(2, m):
    for j in range(2, n):
        up = M[i-1][j]+gap
